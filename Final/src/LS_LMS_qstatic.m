...
for operation = 1 : total_operation
    ...
    for n = 1 : total_length
        ...
        if n <= train_length                                            % compute the error
            ...
            e_1(idx_total) = QPSK_decision(y_1) - y_1;                  
...
% title("Training error for Quasi Static Channel Case 1");
% xlabel("bits");
% ylabel("|e|^2");
% figure
% plot(1:interval:train_length * total_operation, J2_training(1:interval:train_length * total_operation))
% title("Training error for Quasi Static Channel Case 2");
% xlabel("bits");
% ylabel("|e|^2");
...
title("Data Square error for Quasi Static Channel Case 1");
xlabel("symbols");
ylabel("|e|^2");
figure
plot(1:interval:data_length * total_operation, J2_data(1:interval:data_length * total_operation))
title("Data Square error for Quasi Static Channel Case 2");
xlabel("symbols");
ylabel("|e|^2");
...
fprintf('Quasi Static Channel Case 1 EVM is %.2f %% \n',evm_1(total_length*total_operation));
fprintf('Quasi Static Channel Case 2 EVM is %.2f %% \n',evm_2(total_length*total_operation));
fprintf('Quasi Static Channel Case 1 BER is %.5e \n',errorCount1/(2*train_length*total_operation));
fprintf('Quasi Static Channel Case 2 BER is %.5e \n',errorCount2/(2*train_length*total_operation));

%% ==================== Save the result ====================
[ans_qstatic_1, ans_qstatic_2] = QPSK_symbol2bit(result_1, result_2);
save(['../ans/ans_qstatic_1.mat'], 'ans_qstatic_1');
%save(['../ans/ans_qstatic_2.mat'], 'ans_qstatic_2');
